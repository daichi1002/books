...
- スケーラビリティがある
- 耐障害性がある
- 外部ストレージを用意するのでコストがかかる
- ネットワークの遅延が発生してしまう

#### 2. スティッキーセッション方式

- 特定のユーザーのリクエストは必ず同じコンテナにルーティングするようにする
- コンテナが不具合を起こした場合、デメリット
- 特定のユーザーで負荷がかかる場合、分散ができない

#### 3. JWT トークン方式

- JWT トークンを用いて色々な情報をクライアント側に保存させる
- スケーラビリティがある
- セキュリティ上で考えることが多くなる(センシティブな情報を保持できない)

### セッションとは

ユーザーがウェブアプリケーションを使用する際の一連の動作や状態のこと。

> クライアントとサーバーで通信を行う場合であれば、クライアントからサーバーへ接続した時点でセッションが始まり、サーバーから切断するとセッションが終了します。この一連の流れを管理することをセッション管理と言います。

### Cookie とは

ウェブサイトがユーザーのブラウザに保存する小さなデータの断片。主にウェブサイトがユーザーを識別し、その訪問履歴や設定を記憶するために使用される。<br>
...
developer tool の application タブなどから cookie に関する情報が見れる

## マイクロサービスアーキテクチャ入門

### マイクロサービスアーキテクチャとは？

アプリケーションを複数個の小さな自立したサービスに分割して設計するアーキテクチャ。それぞれのサービスは独立していて、特定のビジネス機能を実装している。

#### Scale Cube

- X axis scalling(水平複製スケール)
- `Y axis scalling(機能分割スケール)`
- Z axis scalling(データ分割スケール)

### マイクロサービスアーキテクチャのメリット・デメリット

|            | マイクロサービスアーキテクチャ                           | モノシリックアーキテクチャ                     |
| ---------- | -------------------------------------------------------- | ---------------------------------------------- |
| メリット   | ・柔軟なスケーラビリティ<br>・開発の俊敏性<br>・耐障害性 | ・開発のシンプルさ<br>・デプロイメントの容易さ |
| デメリット | ・複雑性<br>・運用の難しさ                               | ・スケーラビリティ<br>・柔軟性の欠如           |
...
- シンプルな構成
- Lambda などの他のサービスからも通信可能

##### デメリット

- サービス間通信状況の観測は不可能

#### 3.Service Connect 機能を使った通信

プロキシを経由した通信方式(1 番新しい通信手法)
...
- プロキシコンテナ文追加の CPU(256vCPU)/メモリ(64MiB)が必要
- Lambda などの他のサービスから通信が不可能

### Private Subnet にサービスを配置する方法

#### NAT ゲートウェイを利用

Public Subnet を経由した通信。Service -> NAT gateway -> Internet gateway -> ECR, S3, etc...
インバウンドは外部からのアクセスを制御できるが、アウトバウンドに対してはできない。（悪意のあるサイトにアクセスした場合、破壊的な攻撃を受けてしまう可能性がある）

#### VPC エンドポイントを利用

NAT ゲートウェイのデメリットを排除したもの。AWS のリソースに限って通信する手法。

|            | NAT ゲートウェイ利用                                                                     | VPC エンドポイント利用                                  |
| ---------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| メリット   | ・幅広いアクセス性<br>・シンプルなセットアップ<br>                                       | ・通信が AWS 内部に留まる<br>・ネットワークセキュリティ |
| デメリット | ・コスト<br>・スケーリングの限界<br>・シングルポイントのリスク<br>・不要な外部通信を許可 | ・限定された AWS サービスへの接続<br>・複雑な設定       |
